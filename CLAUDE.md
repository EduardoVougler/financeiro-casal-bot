...
- **Lançamento gravado é editável e removível — por conversa, e sempre com confirmação.** "apaga o 3", "muda o valor do 2 para 154,90", "aquele uber era da Duda". Isso impõe três coisas:
  ...
  Na hora de aplicar a edição, **se a instrução não nomear a pessoa, o `autor` atual permanece**: sem essa trava, `applyDono` deixaria o dono do banco vencer e uma fatura dividida ("BB: 666 Eduardo / 809 Duda") voltaria inteira para o dono do cartão na primeira edição de valor. Edição não muda o mês do lançamento (mudar de mês seria mover de arquivo) — por isso `competencia` fica fora dos campos editáveis.
...
3. **`extract.js`** — texto/foto → via Claude + `json_schema` (raiz `{ intencao, mes, ano, lancamentos: [...] }`, cada item com `competencia`). `readFromText` devolve o objeto inteiro (é o classificador de intenção **e** o extrator); `readFromImage` devolve só a lista (foto é sempre lançamento). Intenção/`mes`/`ano` fora do formato caem no default (lançamento, período corrente) em vez de virar leitura errada. Também expõe `applyCorrection` (correção em linguagem natural sobre a lista); `applyDono` resolve o autor por precedência **pessoa citada > dono do banco > quem enviou**. `contextoDeHoje()` injeta a data corrente em toda chamada (texto, foto e correção).
4. **`transcribe.js`** — áudio (OGG) → texto via Groq Whisper; o texto volta para `extract.js`.
5. **`domain.js`** — bancos (Nubank/BB → Maria; Inter/Bradesco → Eduardo), `resolveBanco`/`donoDoBanco`, apelidos de pessoas (`resolvePessoa`), sementes de categoria + `ALIASES_CATEGORIA`/`normalizeCategoria`. **É aqui que se adiciona banco ou apelido** — é dado, não código espalhado. Categoria não: ela se adiciona sozinha, escrevendo no Telegram.
6. **`store.js`** — persistência JSON: `state.json` e `months/<YYYY-MM>.json`; `monthKey`/`previousMonthKey`, `addLancamento`, `yearKey`/`loadYear` (consolida os 12 meses para o anual) e `categoriasUsadas()` (vocabulário vivo de categorias, lido pelo `extract.js`).
